defmodule Indexer.Fetcher.ValidatorStability do
  ...
  def start_link(_) do
    dbg("start link")
    GenServer.start_link(__MODULE__, [], name: __MODULE__) |> dbg()
  end
  ...
  def handle_cast(:update_validators_list, state) do
    dbg("dbg")
    validators_from_db = ValidatorStability.get_all_validators() |> dbg()
      ...
      %{active: active_validator_addresses_list, all: validator_addresses_list} ->
        ...
        address_hashes_to_drop_from_db =
          Enum.flat_map(validators_from_db, fn validator ->
            if is_nil(validators_map[validator.address_hash.bytes]) do
        ...
        active =
          Enum.map(grouped[true] || [], fn address_hash ->
            {:ok, address_hash} = AddressHash.load(address_hash)

            %{
              address_hash: address_hash,
              state:
                ValidatorStability.missing_block_number_to_state(validators_to_missing_blocks_numbers[address_hash])
            }
